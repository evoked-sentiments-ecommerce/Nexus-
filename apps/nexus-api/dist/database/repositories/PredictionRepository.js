...
class PredictionRepository {
    ...
    async list(filter) {
        ...
        const result = await this.queryFn(`SELECT * FROM predictions ${where} ORDER BY created_at DESC`, values);
        return result.rows.map(mapRow);
    }
    async getById(id) {
        const result = await this.queryFn("SELECT * FROM predictions WHERE id = $1 LIMIT 1", [id]);
        return result.rows[0] ? mapRow(result.rows[0]) : null;
    }
    async create(prediction) {
        const result = await this.queryFn(`INSERT INTO predictions (
    ...
    async updateActualOutcome(id, actualOutcome, accuracyScore) {
        const result = await this.queryFn(`UPDATE predictions
       SET actual_outcome = $1, accuracy_score = $2, updated_at = NOW()
       WHERE id = $3
       RETURNING *`, [actualOutcome, accuracyScore, id]);
        return result.rows[0] ? mapRow(result.rows[0]) : null;
    }
}
