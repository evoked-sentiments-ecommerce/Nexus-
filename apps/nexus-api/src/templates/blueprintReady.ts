...
export function blueprintReadySubject(data: BlueprintReadyData): string {
...
export function blueprintReadyHtml(data: BlueprintReadyData): string {
  const {
    customerName,
    blueprintTitle,
    blueprintId,
...
export function blueprintReadyText(data: BlueprintReadyData): string {
  ...
  if (cuisineType) lines.push(`Cuisine Type:  ${cuisineType}`);
  lines.push(`Generated:     ${createdAt.toISOString()}`);
  lines.push("", `View your blueprint: ${viewUrl}`);
  if (downloadUrl) lines.push(`Download PDF:  ${downloadUrl}`);
