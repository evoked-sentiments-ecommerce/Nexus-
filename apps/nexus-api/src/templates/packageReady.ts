...
export function packageReadySubject(data: PackageReadyData): string {
...
export function packageReadyHtml(data: PackageReadyData): string {
  const {
    customerName,
    packageName,
    packageId,
...
export function packageReadyText(data: PackageReadyData): string {
  const {
    customerName,
    packageName,
    packageId,
  ...
  if (expiresAt) lines.push(`Expires:     ${expiresAt.toISOString()}`);
