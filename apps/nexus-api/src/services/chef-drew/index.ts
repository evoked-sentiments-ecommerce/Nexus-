...
export async function generateMenu(ctx: HospitalityContext, sectionNames: string[]): Promise<Menu> {
  logInfo("chef_drew_menu_generated", { operation: ctx.operationName, sections: sectionNames.length });

  const sections: MenuSection[] = sectionNames.map((name) => ({
    name,
    items: [
...
export async function generateRecipe(name: string, portions: number, targetFoodCostPct: number): Promise<Recipe> {
  logInfo("chef_drew_recipe_generated", { name, portions });
...
export async function generateCostModel(ctx: HospitalityContext): Promise<CostModel> {
  logInfo("chef_drew_cost_model_generated", { operation: ctx.operationName });
...
export async function generateProductionSchedule(date: string, shift: ProductionSchedule["shift"], recipeTasks: Array<{ recipeName: string; quantity: number; unit: string; role: string }>): Promise<ProductionSchedule> {
  logInfo("chef_drew_production_schedule_generated", { date, shift });
  return {
...
export async function generateTrainingProgram(role: string, domain: HospitalityDomain): Promise<TrainingProgram> {
  logInfo("chef_drew_training_program_generated", { role, domain });
  return {
...
export async function generateSOPManual(department: string, domain: HospitalityDomain): Promise<SOPManual> {
  logInfo("chef_drew_sop_manual_generated", { department, domain });
  return {
...
export async function generateHACCPPlan(facilityName: string, processDescription: string): Promise<HACCPPlan> {
  logInfo("chef_drew_haccp_plan_generated", { facilityName });
  return {
...
export async function generateBlueprint(ctx: HospitalityContext): Promise<HospitalityBlueprint> {
  logInfo("chef_drew_blueprint_generated", { operation: ctx.operationName, domain: ctx.domain });
  return {
...
function buildMenuItem(section: string, name: string, price: number, cost: number): MenuItem {
...
function buildIngredient(name: string, qty: number, unit: string, unitCost: number, yieldPct: number): RecipeIngredient {
