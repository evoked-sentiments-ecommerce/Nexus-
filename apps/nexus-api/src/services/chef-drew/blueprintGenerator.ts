...
export interface BlueprintGenerationResult {
  ...
  pdfResult: { url: string; key: string; size: number; generatedAt: Date } | null;
}
...
export async function generateBlueprint(
  ...
  logInfo("blueprint_generation_started", { operation: context.operationName, domain: context.domain });

  const agentResults = await Promise.all([
    runAgent({ taskId: `${sid}-hospitality`, agentName: "hospitality_architect", taskType: "create_concept", input: { context }, sessionId: sid }),
    runAgent({ taskId: `${sid}-financial`, agentName: "financial_architect", taskType: "build_financial_model", input: { context }, sessionId: sid }),
    runAgent({ taskId: `${sid}-training`, agentName: "training_architect", taskType: "create_training_outline", input: { context }, sessionId: sid }),
    runAgent({ taskId: `${sid}-operations`, agentName: "operations_architect", taskType: "design_operations", input: { context }, sessionId: sid }),
  ]);

  const trainingSummary = String((agentResults[2].output as Record<string, unknown> | undefined)?.summary ?? "Comprehensive training program");

  const blueprint: HospitalityBlueprint = {
    blueprintId: `bp-${Date.now()}`,
    operationName: context.operationName,
    domain: context.domain,
    concept: String((agentResults[0].output as Record<string, unknown> | undefined)?.summary ?? `${context.operationName} concept`),
    ...
    operationalModel: String((agentResults[3].output as Record<string, unknown> | undefined)?.summary ?? `${context.domain} operating model`),
  ...
  try {
    ...
    logError("blueprint_pdf_error", { message: err instanceof Error ? err.message : String(err) });
  }

  logInfo("blueprint_generation_complete", { blueprintId: blueprint.blueprintId });
