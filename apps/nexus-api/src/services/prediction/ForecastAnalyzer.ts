...
export class ForecastAnalyzer {
  selectPrimaryMetric(
    ...
    const modelMetricMap: Record<string, string> = {
    ...
    const typeMetricMap: Partial<Record<PredictionType, string>> = {
      Revenue: "Revenue",
  ...
  estimateExpectedCosts(predictionType: PredictionType, metrics: PredictionMetric[]): number {
    ...
    if (costMetric != null) return Number(costMetric.toFixed(2));
  ...
  estimateExpectedReturns(
    ...
    if (revenueMetric != null) return Number((revenueMetric - baselineValue).toFixed(2));
